def analyze_string(s: str):
    ...
    element_map: dict[str, int] = {
        element: idx for idx, element in enumerate(unique_elements)
    }
    transformed_elements: list[str] = [
        str(element_map[element]) for element in elements
    ]

    # 將轉換後的元素拼接成字串
    transformed_string: str = "".join(transformed_elements)
