...
class Ticket(Build):
    ...
    class CloseView(discord.ui.View):
        ...
        @discord.ui.button(
            label = "關閉表單", style = discord.ButtonStyle. red, emoji = "🔒", custom_id = "close")
        # pylint: disable-next = unused-argument
        async def button_callback(self, button, interaction):
    ...
    class TicketView(discord.ui.View):
        ...
        async def create_ticket_channel(self, interaction, button_name):
            ...
            if existing_channels:
                await interaction.response.send_message("你已經有建立頻道了！", ephemeral = True)
                return
            ...
            await channel.send(
                f"這裡是{user.mention}的頻道",
                embed = embed,
                view = Ticket.CloseView()
            ) # 修改這裡，使用 Ticket.CloseView()

            await interaction.response.send_message(f"已建立 {channel.mention}！", ephemeral = True)
    ...
    async def create_ticket_button(self, ctx):
        if ctx.author.guild_permissions.administrator:
            ...
            embed.add_field(name = " ", value = " ", inline=False)
            embed.add_field(
                name = "各種伺服器內疑難雜症：包括但不限於 不當言行檢舉、領獎、活動轉發、贊助", value = "", inline = False)
            embed.add_field(name = " ", value = " ", inline=False)
            embed.add_field(name = "課程問題：當你對中電會課程的報名、上課通知有疑慮時可以點我詢問", value="", inline = False)
            embed.add_field(name = " ", value = " ", inline = False)
            embed.add_field(
                name = "----------------- 注意事項 --------------------", value = " ", inline = False)
            embed.add_field(name = " ", value = " ", inline = False)
