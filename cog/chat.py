...
class Chat(commands.Cog):
    ...
    Attributes:
        ...
        client (genai.Client): Gemini API 客戶端，
            未設定 GEMINI_API_KEY 時為 None。
        history (defaultdict): 頻道 ID 對應到該頻道的對話歷史。
        last_used (dict): 使用者 ID 對應到上次對話的時間戳。
    """
    ...
    @staticmethod
    def get_chat_nick(user_id):
        """
        讀取使用者購買的專屬稱呼（/chat_nick 特權）。

        Parameters:
            user_id (int): Discord 使用者 ID。

        Returns:
            str | None: 專屬稱呼，未購買或資料庫無法連線時為 None。
        """
    ...
    async def _generate_gemini(self, model, contents):
        """
        呼叫 Gemini 模型。

        Parameters:
            model (str): 模型名稱。
            contents (list): 要傳給模型的對話內容。

        Returns:
            tuple: (回覆文字, 實際模型, 輸入 token 數, 輸出 token 數)。
        """
    ...
    async def _generate_groq(self, contents):
        """
        呼叫 Groq（OpenAI 相容介面），把 Gemini 格式的對話轉換過去。

        Parameters:
            contents (list): Gemini 格式（types.Content）的對話內容。
    ...
    async def chat(self, message, content):
        """
